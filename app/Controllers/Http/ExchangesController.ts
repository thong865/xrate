...
export default class ExchangesController {
  ...
  public async TxnTradCurrency({ request, response }: HttpContextContract) {
    ...
    const result_curr = await MCcyRate.query().whereIn('ccy',[payload.frm_ccy,payload.to_ccy])
    console.log((result_curr.find((t)=> t.ccy == payload.frm_ccy)?.$attributes[rate_type] / result_curr.find((t)=> t.ccy == payload.to_ccy)?.$attributes[rate_type])*payload.amount)
  }
  ...
  public async SimpleTradCurrency({ request, response }: HttpContextContract) {
    ...
    let rate_type = payload.rate_type == 'B' ? 'buy_rate' : 'sell_rate'
    const result_curr = await MCcyRate.query().whereIn('ccy',[payload.frm_ccy,payload.to_ccy])
    let result = (result_curr.find((t)=> t.ccy == payload.frm_ccy)?.$attributes[rate_type] / result_curr.find((t)=> t.ccy == payload.to_ccy)?.$attributes[rate_type])*payload.amount
    let txn = [
      {
        ref_seq: 1,
